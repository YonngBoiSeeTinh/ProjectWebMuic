...
const deleteSong = async (id) => {
    try {
        ...
        const playlistUpdateResult = await Playlist.updateMany(
            { songs: id }, // Tìm các playlist có chứa bài hát
            { $pull: { songs: id } } // Loại bỏ bài hát khỏi mảng songs
        );

        console.log('Playlist update result:', playlistUpdateResult);
        const userHistory = await User.updateMany(
            { songHistory: id }, // Tìm các playlist có chứa bài hát
            { $pull: { songHistory: id } } // Loại bỏ bài hát khỏi mảng songs
        );
        // console.log('userHistory update result:', userHistory);
        // const userFavorite = await User.updateMany(
        //     { songFavorite: id }, // Tìm các playlist có chứa bài hát
        //     { $pull: { songFavorite: id } } // Loại bỏ bài hát khỏi mảng songs
        // );
        // console.log('userHistory update result:', userFavorite);
        // return {
        //     status: 'OK',
