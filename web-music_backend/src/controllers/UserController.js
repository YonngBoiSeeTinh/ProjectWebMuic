...
const updateUser = async (req, res) => {
    try {
        ...
        const { name, email, password, confirmPassword, phone, isAdmin,address, avatar, sex, introduce,birthday,songHistory,songFavorite } = req.body;
        ...
        const updatedData = { name, email, phone,password,confirmPassword, isAdmin,address, avatar, sex, introduce, birthday,songHistory,songFavorite };
...
const getAllUsers = async (req, res) => {
    try {
        ...
        const response = await UserService.getAllUsers(sort,filter); // Gọi hàm getAllUsers từ UserService
        return res.status(200).json(response);
    } catch (error) {
        return res.status(500).json({
            status: 'ERROR',
