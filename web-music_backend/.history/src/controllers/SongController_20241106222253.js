...
const checkSong = async (req, res) => {
  ...
  try {
    ...
    const arrayBuffer = await responseFile.arrayBuffer();  // Dùng arrayBuffer thay vì buffer()

    // Chuyển đổi arrayBuffer thành Base64
    const audioFileBase64 = Buffer.from(arrayBuffer).toString('base64');
    ...
    const response = await axios.post(`https://${host}/v1/identify`, payload, {
      ...
      console.error("ACRCloud API Error:", err.response ? err.response.data : err.message);
    ...
    console.error("Error in checking copyright:", error.message); // Log chi tiết lỗi
    if (error.response) {
