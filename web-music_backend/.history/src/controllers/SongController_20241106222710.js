...
const checkSong = async (req, res) => {
  try {
    ...
    const buffer = await responseFile.buffer();  // Đọc dữ liệu dưới dạng buffer

    // Nếu cần, bạn có thể lưu file vào ổ đĩa để debug
    fs.writeFileSync('audio.mp3', buffer);  // Lưu tạm thời nếu cần

    // Chuyển dữ liệu âm thanh thành Base64
    const audioFileBase64 = buffer.toString('base64');

    // Gửi yêu cầu tới Audd.io API để kiểm tra bài hát
    const response = await axios.post('https://api.audd.io/', {
      audio: audioFileBase64,  // Dữ liệu Base64 của bài hát
