...
const fs = require("fs");
const fetch = require("node-fetch");  // Dùng node-fetch để tải file từ Firebase
...
const checkSong = async (req, res) => {
  ...
  try {
    const { audioFileUrl } = req.body;  // Lấy URL file nhạc từ client
    // Tải file nhạc từ Firebase Storage về
    ...
    const signature = Buffer.from(access_key + access_secret + timestamp).toString("base64");

    // Gửi yêu cầu tới ACRCloud để kiểm tra bản quyền
    const response = await axios.post(`https://${host}/v1/identify`, audioFileBase64, {
      headers: {
