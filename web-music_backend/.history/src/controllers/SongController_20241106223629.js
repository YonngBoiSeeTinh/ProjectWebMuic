...
const checkSong = async (req, res) => {
  try {
    ...
    const response = await axios.post('https://api.audd.io/', {
      ...
      api_token: 'your_api_token',  // Thay bằng token của bạn từ Audd.io
    });

    const result = response.data.result;
    if (response.data.status === 'success' && result) {
      const { artist, title, album, release_date, song_link } = result;

      // Kiểm tra bài hát có được nhận dạng đúng không
      if (artist && title) {
        console.log(`Song recognized: ${title} by ${artist}`);
        // Trả về thông tin bài hát cho client
        res.json({
        ...
        res.status(400).send("Song recognition failed, no valid data found.");
      }
    } else {
      res.status(404).send("Song not recognized or not found.");
    }
  } catch (error) {
    console.error("Error in checking copyright:", error.message);
    if (error.response) {
      console.error("Response error:", error.response.data); // Log response nếu có
    }
    if (error.request) {
      console.error("Request error:", error.request); // Log request nếu có
    }
