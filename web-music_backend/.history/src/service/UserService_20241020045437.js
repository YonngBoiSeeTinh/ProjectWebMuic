...
const getAllUsers = async (sort, filter) => {
    try {
        let query = {};
        if (filter) {
            const label = filter[0];
            const value = String(filter[1]);
            query = { [label]: { value, '$options': 'i' } };
        }

        const totalUser = await User.countDocuments(query); // Đếm người dùng theo điều kiện lọc (nếu có)
        ...
        if (sort) {
            ...
            userQuery = userQuery.sort(objectSort);
        }

        const users = await userQuery; // Tìm  người dùng
        return {
            status: 'OK',
